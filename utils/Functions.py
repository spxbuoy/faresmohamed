...
buy_button = InlineKeyboardMarkup(
    [[InlineKeyboardButton("Buy Now", url="https://t.me/akatsukichk/1683")]]
)
...
def symbol(symbol: str) -> str:
    href = f"<a href='https://t.me/unknowsyt_bot?start=start'>{symbol}</a>"
    return href
...
async def get_bin_info(bin: str) -> dict | None:
    ...
    async with AsyncClient(follow_redirects=True, verify=False) as s:
        response = await s.get(
            f"https://dpp-bins.vercel.app/bins/{bin}?token=44205_evBDwUnGucBPTJcf5pwR62QkEnjNMT==",
        )

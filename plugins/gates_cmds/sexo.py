...
@Client.on_message(filters.command("sexo", PREFIXES))
async def sexoo(client: Client, m: Message):
    ...
    if not ccs:
        return await m.reply(
            "𝙂𝙖𝙩𝙚𝙬𝙖𝙮 <code>𝙎𝙚𝙭𝙤 -» $0 ♻️</code>\n𝙁𝙤𝙧𝙢𝙖𝙩 -» <code>/sexo cc|month|year|cvc</code>",
            quote=True,
        )
    ini = perf_counter()
    cc = ccs[0]
    ...
    try:
        response = await sexo(cc, mes, ano, cvv)
    except (tenacity.RetryError, IOError):
        response = None

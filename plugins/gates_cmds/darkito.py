...
@Client.on_message(filters.command("dkt", PREFIXES))
async def dkt(client: Client, m: Message):
    ...
    if not ccs:
        return await m.reply(
            "𝙂𝙖𝙩𝙚𝙬𝙖𝙮 <code>𝔻𝔸ℝ𝕜𝟜𝕀𝕋𝕆  ♻️ -» $28.57</code>\n𝙁𝙤𝙧𝙢𝙖𝙩 -» <code>/dkt cc|month|year|cvc</code>",
            quote=True,
        )
    ini = perf_counter()
    cc = ccs[0]
    ...
    status, result = await darkito(cc, mes, ano, cvv)
...
⸙ 𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» <code>𝔻𝔸ℝ𝕜𝟜𝕀𝕋𝕆 -» $28.57</code>

...
@Client.on_message(filters.command("dj", PREFIXES))
async def dj(client: Client, m: Message):
    ...
    if not ccs:
        return await m.reply(
            "𝙂𝙖𝙩𝙚𝙬𝙖𝙮 <code>𝘿𝙟 𝘽𝙖𝙗𝙮 ♻️ -» $9.95 -» vbv</code>\n𝙁𝙤𝙧𝙢𝙖𝙩 -» <code>/dj cc|month|year|cvc</code>",
            quote=True,
        )
    ini = perf_counter()
    cc = ccs[0]
    ...
    try:
        status, response, vbv = await djbaby(cc, mes, ano, cvv)
...
⸙ 𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» <code>𝘿𝙟 𝘽𝙖𝙗𝙮 -» $9.95</code>
